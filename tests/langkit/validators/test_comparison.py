...
def test_one_required():
    with pytest.raises(Exception):
        ConstraintValidator("prompt.char_count")


def test_upper_threshold():
    validator = ConstraintValidator("prompt.char_count", upper_threshold=5)
    wf = EvaluationWorkflow(metrics=[metric_lib.text_stat.char_count.prompt], validators=[validator])
...
def test_lower_threshold():
    validator = ConstraintValidator("prompt.char_count", lower_threshold=5)
    wf = EvaluationWorkflow(metrics=[metric_lib.text_stat.char_count.prompt], validators=[validator])
...
def test_upper_threshold_inclusive():
    validator = ConstraintValidator("prompt.char_count", upper_threshold_inclusive=5)
    wf = EvaluationWorkflow(metrics=[metric_lib.text_stat.char_count.prompt], validators=[validator])
...
def test_lower_threshold_inclusive():
    validator = ConstraintValidator("prompt.char_count", lower_threshold_inclusive=5)
    wf = EvaluationWorkflow(metrics=[metric_lib.text_stat.char_count.prompt], validators=[validator])
...
def test_one_of():
    validator = ConstraintValidator("prompt.char_count", one_of=[1, 2, 3])
    wf = EvaluationWorkflow(metrics=[metric_lib.text_stat.char_count.prompt], validators=[validator])
...
def test_none_of():
    validator = ConstraintValidator("prompt.char_count", none_of=[1, 2, 3])
    wf = EvaluationWorkflow(metrics=[metric_lib.text_stat.char_count.prompt], validators=[validator])
...
def test_must_be_none():
    ...
    wf = EvaluationWorkflow(metrics=[metric_lib.pii_presidio.prompt()], validators=[validator])

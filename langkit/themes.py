...
def register_theme_udfs():
    ...
    if "jailbreaks" in _theme_groups:
        register_dataset_udf([_prompt], namespace="security")(known_jailbreak_similarity)
    else:
        diagnostic_logger.info("No jailbreaks found in theme groups file")

    if "refusals" in _theme_groups:
        register_dataset_udf([_response], namespace="security")(refusal_similarity)
    else:
        diagnostic_logger.info("No refusals found in theme groups file")


def known_jailbreak_similarity(strings) -> list:
    ...
    for text in strings:
        try:
            ...
            score = max(similarities) if similarities else None
            series_results.append(score)
        except Exception as e:
            diagnostic_logger.warn(
                f"Exception in themes jailbreak udf: {e}"
            )
...
def refusal_similarity(strings) -> list:
    if _transformer_model is None:
        raise ValueError("Must initialize a transformer before calling encode!")
    series_results = []
    for text in strings:
